...
class MultiActor(mp.Process):
    def __init__(self, actor_id, shared_state,shared_replay_mem, actor_params):
...
class Learner(object):
    def __init__(self, learner_params, shared_state, shared_replay_memory):
    ...
    def learn(self):
        ...
        for episode in range(NUM_EPISODES):
            ...
            for step in range(MAX_TASKS-1):
                ...
                self.replay_memory.add(priorities, prioritized_xp_batch)
...
if __name__ == '__main__':
    ...
    learner = Learner(learner_params, shared_state, replay_mem)
    learner_proc = mp.Process(target=learner.learn)
    learner_proc.start()

    #  TODO: Test with multiple actors
    actor_procs = []
    for i in range(actor_params["num_actors"]):
        actor_proc = MultiActor(i, shared_state, shared_mem, actor_params)
        actor_proc.start()
