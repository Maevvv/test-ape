...
class RainbowDQN(nn.Module):
    ...
    def decide_action(self,state,episode):
        ...
        if epslion<= np.random.uniform(0,1):
            target_model.eval()
            with torch.no_grad():
                action=target_model(state).max(1)[1].view(1,1)
        else:
...
class MultiActor(mp.Process):
    ...
    def epsilon_greedy_Q(self, qS_t):
        if random.random() >= self.epsilon:
            #print("np.max(qS_tpn,1):", np.argmax(qS_t,1))
            #print("atshape",qS_t)
    ...
    def compute_priorities(self, n_step_transitions):
        ...
        A_t = np.array(n_step_transitions.A_t)#当前动作
        ...
        # with torch.no_grad():
        #     Q_double = target_model(torch.from_numpy(S_tpn).float())
        ...
        # G_t_0 = G_t[:, 0]
        # print("G_t0",G_t_0)
        # print("G_t0",G_t_0.shape)

        # # print("S_t.shape",S_t.shape)
        # # print("S_t.shape",S_t)
        # # print("current_model(S_t).shape",current_model(S_t).shape)
        # # #print("current_model(S_t)",current_model(S_t))
        # # print("current_model(S_t)[2].shape",current_model(S_t)[1].shape)
        # # print("current_model(S_t)[2]",current_model(S_t)[1])
        # #print("current_model(S_t)[A_T].shape",current_model(S_t)[A_t].shape)
        # Q = current_model(S_t)[1]
        # Q = current_model(S_t)[1].reshape(5,-1)
        ...
        # #A_t = A_t.unsqueeze(-1)
        # # print("A1",A_t)
        # # print("A_T_S",A_t.shape)
        # # print("A_T_S",A_t.view(-1,1))
        # # print("A_T_S",A_t.view(-1,1).shape)       
